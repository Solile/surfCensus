...
namespace sgm_util
{
    ...
    void census_transform_dx2d(const uint8 *source, uint64 *census, const sint32 &width, const sint32 &height);

    // Hamming距离
    uint8 Hamming32(const uint32 &x, const uint32 &y);
    uint8 Hamming64(const uint32 &x, const uint32 &y);
    ...
    */
    void CostAggregateLeftRight(const uint8* img_data, const sint32& width, const sint32& height, const sint32& min_disparity, const sint32& max_disparity,
	    const sint32& p1, const sint32& p2_int, const uint8* cost_init, uint8* cost_aggr, bool is_forward = true);
    ...
    */
     ...
     * \param min_speckle_area  输入，最小连通区面积
