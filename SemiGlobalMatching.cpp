...
bool SemiGlobalMatching::Match(const uint8 *img_left, const uint8 *img_right, float32 *disp_match, std::vector<std::pair<int, int>> left_matches, float32 *disp_left)
{
    if (!is_Initialized_)
    {
        return false;
    ...
    left_matches_ = left_matches;
    disp_match_ = disp_match;
    ...
    {
        sgm_util::RemoveSpeckles(disp_left_, width_, height_, 1, option_.min_speckle_area, Invalid_Float);
    }
    // 视差填充
...
void SemiGlobalMatching::CensusTransform() const
{
    ...
    else if (option_.census_size == Census9x7)
    {
    ...
    {
        sgm_util::census_transform_dx2d(img_left_, static_cast<uint64 *>(census_left_), width_, height_);
        sgm_util::census_transform_dx2d(img_right_, static_cast<uint64 *>(census_right_), width_, height_);
    }
};
...
void SemiGlobalMatching::ComputeDisparity() const
{
    ...
    const float32 uniqueness_ration = option_.uniqueness_ration;
    ...
    for (sint32 i = 0; i < height; i++)
    {
        for (sint32 j = 0; j < width; j++)
        {
            ...
            uint16 min_cost_second = UINT16_MAX;
            ...
            if (is_check_unique)
            {
                for (sint32 d = min_disparity; d < max_disparity; d++)
                {
                    ...
                    min_cost_second = std::min(min_cost_second, cost);
                }

                // 判断唯一性约束
                // 若(min-sec) < min*(1-uniquness)，则为无效估计
                if (min_cost_second - min_cost <= static_cast<uint16>(min_cost * (1 - uniqueness_ration)))
                {
            ...
            // {
            //     disp_left_[i * width + j] = static_cast<float32>(best_disparity);
            // }
            // else
            // {
            //     disp_left_[i * width + j] = Invalid_Float;
            // }
...
void SemiGlobalMatching::CompareDisparity() const
{
    // disp_match 通过SLAM的到的视差disp_match[i * width + j] = d;
    // left_matches_ 匹配成功的点的坐标
    // disp_left_ 代价聚合得出的视差图

    const sint32 width = width_;
    const sint32 height = height_;
    std::vector<std::vector<int>> error(height, std::vector<int>(width, 0.0f));
    sint32 countPositive = 0, countNegative = 0;
    for (sint32 i = 0; i < height; i++)
    {
        for (sint32 j = 0; j < width; j++)
        {
            auto &disp = disp_left_[i * width + j];
            auto disp_m = disp_match_[i * width + j];
            error[i][j] = Invalid_Float;
            if (disp_m != Invalid_Float)
            {
                if (abs(disp - disp_m) <= 1e-1)
                {
                    return;
                }
                else
                {
                    error[i][j] = abs(disp - disp_m);
                    if (disp > disp_m)
                    {
                        countPositive++;
    ...
    std::cout << "The count positive is " << countPositive << std::endl;
    std::cout << "The count Negative is " << countNegative << std::endl;
};
        ...
        for (sint32 j = 0; j < width; j++)
        {
            ...
            for (sint32 d = min_disparity; d < max_disparity; d++)
            {
                ...
                if (col_left >= 0 && col_left < width)
                {
                    ...
                    cost_local[d_idx] = cost_ptr[i * width * disp_range + col_left * disp_range + d_idx];
                    if (min_cost > cost)
                    {
                        min_cost = cost;
                        best_disparity = d;
    ...
    for (sint32 i = 0; i < height; i++)
    {
        for (sint32 j = 0; j < width; j++)
        {
            ...
            if (col_right >= 0 && col_right < width)
            {
                ...
                // 判断视差值是否一致（在阈值范围内）|dlr(x, y) - dlr(x + dlr(x, y), y)| < T ?
                if (abs(disp - disp_r) > threshold)
                {
                    // 区分遮挡区和误匹配区
                    // 通过右影像视差算出左影像的匹配像素，并获取视差disp_rl
                    // if(disp_rl > disp)
                    //      pixl in occlusions
                    // else
                    //      pixl in mismatches
...
void SemiGlobalMatching::FillHolesDispMap()
{
    const sint32 width = width_;
    const sint32 height = height_;
