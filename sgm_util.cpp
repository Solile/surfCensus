...
void sgm_util::census_transform_9x7(const uint8* source, uint64* census, const sint32& width,
	const sint32& height)
{
	if (source == nullptr || census == nullptr || width <= 9u || height <= 7u)
	{
		return;
	}
...
void sgm_util::census_transform_dx2d(const uint8 *source, uint64 *census, const sint32 &width, const sint32 &height)
{
	// d = max(x0-x3);
	sint32 d = 10;
	if (source == nullptr || census == nullptr || width <= 4 * d + 1||height <= 2*d + 1)
	{
		return;
	}
...
uint8 sgm_util::Hamming64(const uint32& x, const uint32& y)
{
...
void sgm_util::CostAggregateLeftRight(const uint8* img_data, const sint32& width, const sint32& height,
	...
	const sint32& p1, const sint32& p2_int,
	const uint8* cost_init, uint8* cost_aggr, bool is_forward)
{
	// 宏，用于在运行时检查一个条件是否为真，如果条件不满足，则运行时将终止程序的执行并输出一条错误信息
	...
	for (sint32 i = 0u; i < height; i++)
	{
		...
		// 路径上上一个像素的代价数组，首尾各一个元素避免边界溢出	|数组里每个都赋值UINT8_MAX
		...
		// 路径上上一个像素的最小代价值
		uint8 min_cost_last_path = UINT8_MAX;
		...
		for (sint32 j = 0; j < width - 1; j++)
		{
			...
			{
				// p像素 C匹配代价 d视差
				// L(p,d) = C(p,d)+ min(L(p-r,d), L(p-r,d-1)+P1, L(p-r,d+1)+P1, minL(p-r,i)+P2_Init) - minL(p-r,i)
				const uint8 cost = cost_init_row[d];
				// const uint8  cost = cost_init_row[d + 1];		// 按照公式的最佳结果
				const uint16 l1 = cost_last_path[d + 1];
				const uint16 l2 = cost_last_path[d] + P1;
				const uint16 l3 = cost_last_path[d + 2] + P1;
				const uint16 l4 = min_cost_last_path + std::max(P1, P2_Int / (abs(gray - gray_last) + 1));

				const uint8 cost_s = cost + static_cast<uint8>(std::min(std::min(l1, l2), std::min(l3, l4)) - min_cost_last_path);
	...
	for (sint32 j = 0u; j < width; j++)
	{
		...
		for (sint32 i = 0u; i < height - 1; i++)
		{
			...
			for (sint32 d = 0; d < disp_range; d++)
			{
				...
				// L(p,d) = C(p,d)+ min(L(p-r,d), L(p-r,d-1)+P1, L(p-r,d+1)+P1, minL(p-r,i)+P2_Init) - minL(p-r,i)
				const uint8 cost = cost_init_col[d];
				// const uint8  cost = cost_init_col[d + 1];		// 按照公式的最佳结果
				const uint16 l1 = cost_last_path[d + 1];
				const uint16 l2 = cost_last_path[d] + P1;
				const uint16 l3 = cost_last_path[d + 2] + P1;
				const uint16 l4 = min_cost_last_path + std::max(P1, P2_Int / (abs(gray - gray_last) + 1));
...
void sgm_util::RemoveSpeckles(float32* disparity_map, const sint32& width, const sint32& height,
	const sint32& diff_insame, const uint32& min_speckle_area, const float32& invalid_val)
{
